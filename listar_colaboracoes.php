...
?>
	...
	<html>
		...
		//Consultas SQL
		$consulta = $connection->query("SELECT * FROM colaboracao ORDER BY datahoraCriacao DESC");
		$numColaboracoes = $consulta->num_rows;
		
		if($cga == '' && $sta == '' && $tipoAtual == '')
			$numColaboracoesAtual = $numColaboracoes;
		else if($cga != '' && $sta != '' && $tipoAtual == ''){
			$consulta5 = $connection->query("SELECT * FROM colaboracao WHERE (codCategoriaEvento = '$cga') AND (tipoStatus = '$sta') ORDER BY datahoraCriacao DESC");
			$numColaboracoesAtual = $consulta5->num_rows;
		}
		else if($cga != '' && $sta == '' && $tipoAtual != ''){
			$consulta5 = $connection->query("SELECT * FROM colaboracao WHERE (codCategoriaEvento = '$cga') AND (codTipoEvento = '$tipoAtual') ORDER BY datahoraCriacao DESC");
			$numColaboracoesAtual = $consulta5->num_rows;
		}
		else if($cga != '' && $sta == '' && $tipoAtual == ''){
			$consulta5 = $connection->query("SELECT * FROM colaboracao WHERE (codCategoriaEvento = '$cga') ORDER BY datahoraCriacao DESC");
			$numColaboracoesAtual = $consulta5->num_rows;
		}
		else if($cga == '' && $sta != '' && $tipoAtual == ''){
			$consulta5 = $connection->query("SELECT * FROM colaboracao WHERE (tipoStatus = '$sta') ORDER BY datahoraCriacao DESC");
			$numColaboracoesAtual = $consulta5->num_rows;
		}
		else if($cga == '' && $sta == '' && $tipoAtual != ''){
			$consulta5 = $connection->query("SELECT * FROM colaboracao WHERE (codTipoEvento = '$tipoAtual') ORDER BY datahoraCriacao DESC");
			$numColaboracoesAtual = $consulta5->num_rows;
		}
		else if($cga == '' && $sta != '' && $tipoAtual != ''){
			$consulta5 = $connection->query("SELECT * FROM colaboracao WHERE (tipoStatus = '$sta') AND (codTipoEvento = '$tipoAtual') ORDER BY datahoraCriacao DESC");
			$numColaboracoesAtual = $consulta5->num_rows;
		}
		else if($cga != '' && $sta != '' && $tipoAtual != ''){
			$consulta5 = $connection->query("SELECT * FROM colaboracao WHERE (codCategoriaEvento = '$cga') AND (tipoStatus = '$sta') AND (codTipoEvento = '$tipoAtual') ORDER BY datahoraCriacao DESC");
			$numColaboracoesAtual = $consulta5->num_rows;
		}
				...
				<form action="#" id="application-form" method="post" name="adminForm" class="form-validate">
							...
							<select name="filtro_tipo" id="filtro_tipo" class="inputbox" onchange="filtro()">
								<?php 	
									if($cga == '')
										...
										$consulta4 = $connection->query("SELECT * FROM tipoevento WHERE codCategoriaEvento = '$cga' ORDER BY desTipoEvento ASC");
										while($tipo = $consulta4->fetch_array()){
					...
					<table class="adminlist">
						...
						<tbody>
							<?php 
							$i = 0;
							while($colaboracao=$consulta->fetch_array()){
								if(($tipoAtual == $colaboracao['codTipoEvento'] || $tipoAtual == '') && ($cga == $colaboracao['codCategoriaEvento'] || $cga == '') && ($sta == $colaboracao['tipoStatus'] || $sta == '')){
									echo '<tr class="row'.($i%2).'">';
										...
										$consulta2 = $connection->query("SELECT * FROM categoriaevento WHERE codCategoriaEvento = '$categoria' ");
										$nomeCategoria=$consulta2->fetch_array();
										echo '<td>';
											echo '<span style="font-size: 0.75em;"><center>'.$nomeCategoria['desCategoriaEvento'].'<center></span>';
										echo '</td>';
										
										$tipo = $colaboracao['codTipoEvento'];
										$consulta3 = $connection->query("SELECT * FROM tipoevento WHERE codTipoEvento = '$tipo' ");
										$nomeTipo=$consulta3->fetch_array();
