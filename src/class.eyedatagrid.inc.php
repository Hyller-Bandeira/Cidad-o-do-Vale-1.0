...
class EyeDataGrid
{
	...
	private $image_path = 'src/images/'; // Path to images
	...
	const TXT_NORESULTS = 'Nenhum resultado foi encontrado!';
	...
	*/
	 ...
	 *
	 * @param string $fields Fields to fetch from table. * for all columns
	...
	*/
	private function buildHeader()
	{
		...
		{
			$this->column_count++;
			echo '<td class="tbl-header">Posição</td>';
		}
		...
		foreach ($headers as $t)
		{
									...
									)))
			{
				...
				echo '<a href="javascript:;" onclick="tblShowHideFilter(\'' . $t . '\')"><img src="' . $this->image_path . 'filter.gif" class="tbl-filter-image"></a><br><div class="tbl-filter-box" id="filter-' . $t . '" style="display:' . $filter_display . '"><input type="text" size="6" id="filter-value-' . $t . '" value="'.$filter_value.'">&nbsp;<a href="javascript:;" onclick="tblSetFilter(\'' . $t . '\')">filtrar</a></div>';
			}*/
	...
	*/
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		echo '<tfoot><tr class="tbl-footer"><td class="tbl-nav" colspan="' . $this->column_count . '"><table width="100%" class="tbl-footer"><tr><td width="33%" class="tbl-found">Foram encontrados <em>' . $this->row_count . '</em> resultados';

		if ($this->row_count > 0)
			echo ', mostrando <em>' . $first . '</em> até <em>' . $last . '</em>';
	...
	*/
				...
				foreach ($row as $key => $value)
				{
					...
					if (array_key_exists($key, $this->type))
					{
						...
						switch ($type)
						{
							...
							case self::TYPE_ARRAY2:
								$value = $criteria[$value];
								if ($value != $_SESSION['name_user'])
									$value='???';
                    ...
                    if($key == 'apelidoUsuario'){
					    echo '<td class="tbl-cell"><a href="user_profile.php?uid='.$value.'">' . $value . '</a></td>';
                    } else {
	 ...
	 */
	public static function useAjaxTable($responce = '', $complemento_get='')
	{
		...
		echo "function updateTable() { xmlHttp = SetXmlHttpObject(); xmlHttp.onreadystatechange = stateChanged; xmlHttp.open('GET', '" . $responce . "?useajax=true".$complemento_get."' , true); xmlHttp.send(null); }\n";
		echo "</script>\n";
