...
class EyeMySQLAdap
{
	 ...
	 * @return boolean
	 */
	/* Modified by Rafael OP */
	public function connect($persist = true)
	{
		if ($persist) $nhost = "p:" . $this->host;
		$link = new mysqli($nhost, $this->user, $this->pass, $this->db_name);

		if (!$link->set_charset('utf8'))
		{
			trigger_error('Error loading character set uft8.', E_USER_ERROR);
			return false;
		}
	...
	{
		$this->resCalc($query);
		return $this->mysqli_field_name($query, $offset);
	}
