...
$consulta = mysql_query("SELECT * FROM  colaboracao WHERE codColaboracao = '$codColaboracao' ");
	...
	if(!($linhaDaConsulta5 = mysql_fetch_assoc($consulta5))){
		...
		$numRevisoes = mysql_num_rows($consulta5) - 1;//Subtrai 1 pois quando a colaboração é criada ela é gravada nessa tabela, ou seja, ela ainda não foi editada mas existe uma linha com seus dados
	...
	if(mysql_num_rows($consulta9)<1){
		...
		$numRevisores = mysql_num_rows($consulta9) - 1;//Subtrai 1 pois o autor não conta como revisor
