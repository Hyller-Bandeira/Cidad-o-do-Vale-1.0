...
?>
	...
	<html>
		...
		//Consultas SQL
		$consulta = mysql_query("SELECT * FROM tipoevento ORDER BY desTipoEvento ASC");
		$numTipos = mysql_num_rows($consulta);
		
		$consulta1 = mysql_query("SELECT * FROM categoriaevento ORDER BY desCategoriaEvento ASC");
		
		if($categoriaAtual != ''){
			$consulta2 = mysql_query("SELECT * FROM tipoevento WHERE codCategoriaEvento = '$categoriaAtual'");
			$numTiposAtual = mysql_num_rows($consulta2);
		}else
				...
				<form action="#" id="application-form" method="post" name="adminForm" class="form-validate">
					...
					<table class="adminlist">
						...
						<tbody>
							<?php 
							$i = 0;
							while($tipo=mysql_fetch_array($consulta)){
								if($categoriaAtual == $tipo['codCategoriaEvento'] || $categoriaAtual == ''){
									echo '<tr class="row'.($i%2).'">';
										...
										$consulta1 = mysql_query("SELECT * FROM categoriaevento WHERE codCategoriaEvento = '$codigo'");
