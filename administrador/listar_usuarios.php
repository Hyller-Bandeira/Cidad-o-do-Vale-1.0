...
?>
	...
	<html>
		...
		//Consultas SQL
		$consulta = $connection->query("SELECT * FROM usuario ORDER BY nomPessoa ASC");
		$numUsuarios = $consulta->num_rows;
		
		$consulta1 = $connection->query("SELECT * FROM categoriaevento ORDER BY desCategoriaEvento ASC");
		
		if($tipoAtual != ''){
			$consulta2 = $connection->query("SELECT * FROM usuario WHERE codUsuario = '$tipoAtual'");
			$numUsuariosAtual = $consulta2->num_rows;
		}else
