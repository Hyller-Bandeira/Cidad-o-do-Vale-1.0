<script type="text/javascript">
	...
	{
		$.get("incrementaVisualizacao.php", {codColaboracao: codColaboracao}, function(data) {});
	}
	...
	function load_marker_filtro(ids_filtros)
	{
		...
		markerCluster = new MarkerClusterer(map, listaCluster, { maxZoom: 15 });
	}
	...
	function open_infowindows_especifico(k, marker)
	{
		downloadUrl_a(base + "phpsqlajax_genxml_especifico.php?idColaboracao=" + k, function(data)
		{
            ...
            var keywords = colaboracao[i].getAttribute('keywords');
			createInfoWindows(point, codCategoriaEvento, codTipoEvento, desTituloAssunto, dataOcorrencia, horaOcorrencia,
							  desColaboracao, tipoStatus, codUsuario, codColaboracao, desTituloImagem, desTituloVideo,
							  desUrlVideo, forum, notaMedia, qtdVisualizacao, qtdAvaliacao ,endArquivo, endImagem,
							  comentarioImagem, comentarioArquivo, tituloArquivo, comentarioVideo, marker, datahoraCriacao, keywords);
			return;
		});
	}
	...
	function limpa_markers()
	{
		for( i = 0; i < listaMarcadores.length; ++i)
			if (listaMarcadores[i]) listaMarcadores[i].setMap(null);
		listaMarcadores = [];
		listaCluster = [];
		pos = 0;
	}
	...
	function showAddress(address)
	{
		geocoder.geocode( { 'address': address}, function(results, status)
		{
			...
			else alert("Geocodificação não foi bem sucedida pelo seguinte motivo: " + status);
		});
	}
	...
	function changeRadius() { heatmap.setOptions({radius: heatmap.get('radius') + 5}); }

	function changeRadius2() { heatmap.setOptions({radius: heatmap.get('radius') - 5}); }

	function changeOpacity() { heatmap.setOptions({opacity: heatmap.get('opacity') ? null : 0.2}); }

	function geraMetadados(id) { $.get("gera_metadados.php", {id: id}, function(data) { $('#metadados').html(data); }); }
</script>