<?php 
	...
	";
	// Executa a query
	$atualiza = $connection->query($query1);
	
	$query2 = "SELECT * FROM configuracoes"; 
	$result2 = $connection->query($query2);
	if (!$result2) 
		 die('Invalid query: ' . $connection->error);
	$escrever2 = $result2->fetch_array();
	...
	if($enviaEmail == 'true'){
		...
		while($escrever=$result->fetch_array()){
			...
			$consultaUsuario = $connection->query("SELECT * FROM usuario WHERE codUsuario = '$temp3'" );
			$rowUsuario = $consultaUsuario->fetch_assoc();
